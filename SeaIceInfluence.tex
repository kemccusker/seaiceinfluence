...
FIGURES:\\
-Latitude by month SAT anomaly plus SIC anomaly contours. Potentially add bar graph of polar average net flux to same fig. For HAD, NSIDC, $\overline{TOT}$ (?), $\overline{ANT}$\\
-Seasonal cycle polar average SIA, NET flux, SAT, SLP, Z500 (and PREC?). This figure will show TOT mean and range, ANT mean, HAD, NSIDC all on one figure.\\
-NH maps of SAT, SLP, Z500 response for HAD, NSIDC, $\overline{TOT}$, $\overline{ANT}$ for SON and DJF.\\
-NH zonal mean temp, zonal wind, geopotential height with altitude in HAD, NSIDC, $\overline{TOT}$, $\overline{ANT}$ for SON and DJF \\

\subsection{Internal variability in the BCs}

Part of the historical sea ice loss is due to internal variability rather than external forcing (up to 40\% cite Kay, Stroeve). \\
...
iv. [NEW] Internal variability in the sea ice pattern amplifies the SON temperature response in NSIDC and HAD in SON (smaller SIC anomalies, larger SAT anomalies). (Look at wind anoms, see if southerly to advect warmer air?)\\
...
vi. In CanAM4, even 120 years was not enough to settle on a converged circulation response even locally. Evidence is that the spread in SLP and Z500 especially in DJF is wider in ANT than in TOT.\\
vii. [NEW] One explanation for wildly variable circulation in the ANT ensemble even in  SON and DJF could be the relative uniformity of the anthropogenic sea ice boundary condition. This may cause the polar vortex to weaken and wobble more and not settle on a pattern (inter-annual standard deviation is *slightly* larger in ANT in Jan at least than in TOT. What does decadal standard dev look like?). In contrast, more regional sea ice forcing (Barents-Kara Seas, Sea of Okhotsk tend to cause rossby wave responses) may force a particular circulation pattern, keeping the spread tighter between ensemble members. In this way, internal variability in the sea ice pattern acts to amplify the response to anthropogenic sea ice loss. This observation is consistent with the fact that there are generally larger areas of significance in the TOT ensemble mean than the ANT ensemble mean (except SLP in SON). \\
